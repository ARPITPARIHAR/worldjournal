...
@include('user.includes.nav')
 <div class="search">
  <h3 class="widgettitle">Search CFP</h3>
   ...
   <form action="{{route('user.search')}}" id="myform" method="get">
         ...
         <span id="category_id_Info"></span><select name="category" class="form-control input-lg margin-0 selectpicker">
          ...
          <option value="4">security</option><option value="5">software engineering</option><option value="6">data mining</option><option value="7">engineering</option><option value="8">networking</option><option value="9">machine learning</option><option value="10">manufacturing</option><option value="11">finance</option><option value="12">JOURNAL</option><option value="13">Management</option><option value="14">Interdisciplinary</option><option value="15">Wireless</option><option value="16">Pharmaceutical</option><option value="17">Humanities And Social Science</option><option value="18">UGC Care Journals</option></select><br/><br/>
         ...
         <button style="margin-left: 20px" type="submit" class="btn btn-primary btn-block" > Search</button>
   </form>
 </div>
  ...
  <div class="push-up">
    <div class="table-responsive">
        ...
        <span>{{ $data->links() }} </span>
          ...
          {{-- {{ $data->appends(Request::only(['issn'=>'issn', 'journal_title'=>'journal_title','publisher_name'=>'publisher_name']))->render() }} --}}
          {{-- {{ $data->appends(['query' => $data])->links() }}</span></span> --}}
      </div>
    </div>
...
margin-bottom: 1400px; border: 1px solid #ccc;
...
border-spacing: 5px; /* 5px ki jagah apne pasand ke pixels ya percentage value set kare */
}
</style>  
@endsection
@section('script')
